export const LINKS =  {
    aboutUs: '/about-us',
    ai: '/docs/ai/ai-intro',
    apiReference: 'https://api-docs.neon.tech/reference/getting-started-with-neon-api',
    autoscaling: '/docs/introduction/autoscaling',
    awsIsrael: '/partners/aws-israel',
    billing: '/docs/introduction/about-billing',
    blog: '/blog',
    branching: '/branching',
    businessInformation: '/business-info',
    careers: '/careers',
    caseStudies: '/case-studies',
    changelog: '/docs/changelog',
    cli: '/cli',
    cliInstall: '/docs/reference/cli-install',
    cliReference: '/docs/reference/neon-cli',
    console: 'https://console.neon.tech',
    contactSales: '/contact-sales',
    cookiePolicy: '/cookie-policy',
    creators: '/creators',
    dashboard: 'https://console.neon.tech/sign_in',
    demos: '/demos',
    developerDays1: '/developer-days-1',
    discord: 'https://discord.gg/92vNTzKDGp',
    discourse: 'https://community.neon.tech',
    discussions: 'https://abhinandan-v.vercel.app/contact',
    docs: '/docs',
    docsHome: '/docs/introduction',
    docsBranching: '/docs/introduction/branching',
    dpa: '/dpa',
    earlyAccess: '/early-access-program',
    enterprise: '/enterprise',
    flow: '/flow',
    github: 'https://github.com/awesome-pro/awesome-landing',
    guides: '/guides',
    linkedin: 'https://www.linkedin.com/in/abhinandan-verema',
    login: 'https://console.neon.tech/login',
    onDemandStorage: '/docs/introduction/about#bottomless-storage',
    partners: '/partners',
    postgres: '/docs/postgres',
    postgresDocs: 'https://www.postgresql.org/docs/',
    postgresList: 'https://www.postgresql.org/list/',
    pricing: '/pricing',
    privacy: '/privacy-policy',
    privacyGuide: '/privacy-guide',
    privacyPolicy: '/privacy-policy',
    scaleTrial: '/scale-trial',
    security: '/docs/security/security-overview',
    signup: '/sign-up',
    subprocessors: '/subprocessors',
    support: '/docs/introduction/support',
    terms: '/terms-of-service',
    termsOfService: '/terms-of-service',
    thankYou: '/thank-you',
    trust: 'https://trust.neon.tech',
    variable: '/variable',
    twitter: 'https://twitter.com/abhinandan_v0/',
    youtube: 'https://www.youtube.com/@typescriptmastery001',
  };
